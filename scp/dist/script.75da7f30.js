...
(function animeAboutSection() {
  ...
  var innerSkillBarLvlArr = _toConsumableArray(innerSkillBar).map(function (bar) {
    return bar.dataset.level;
  });

  var style = document.createElement('style');

  createAnimationPerSkillsLevel = function createAnimationPerSkillsLevel(lvl) {
    return "\n    @keyframes growFromLeft".concat(lvl, "{\n      from{\n        width: 0;\n      }\n\n      to{\n        width:").concat(lvl, "%;\n      }\n    }\n  ");
  };

  style.innerHTML = innerSkillBarLvlArr //Remove duplicate first
  // .filter((a,b) => this.indexOf(a) === b)
  //Then create the animation rules.
  .map(function (lvl) {
    return createAnimationPerSkillsLevel(lvl);
  }).join(' '); // Get the first script tag
  ...
  window.addEventListener('scroll', function () {
    ...
    if (skills.getBoundingClientRect().y < window.innerHeight && work.getBoundingClientRect().y > 0) {
      innerSkillBarArr.forEach(function (bar) {
        return bar.style.animation = "growFromLeft".concat(bar.dataset.level, " 2s ease-in-out");
      });
      innerSkillBarArr.forEach(function (bar) {
        return bar.style.animationFillMode = 'forwards';
      });
    } else {
      innerSkillBarArr.forEach(function (bar) {
        return bar.style.animation = "";
      });
      innerSkillBarArr.forEach(function (bar) {
        return bar.style.animationFillMode = '';
      });
    }
  });
...
var worksProjects = _toConsumableArray(document.querySelectorAll('.works__project'));

var buttonCloseModal = document.querySelector('button'); //Flex - Grow the project

worksProjects.map(function (project) {
  project.querySelector('img').addEventListener('click', function (event) {
    worksProjects.map(function (project) {
  ...
  project.querySelector('.project__btnClose').addEventListener('click', function () {
    worksProjects.map(function (project) {
      project.classList.remove('featuredProject');
    });
  });
